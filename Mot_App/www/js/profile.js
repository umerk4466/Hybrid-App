...
$(document).ready(function() {
  ...
  getUserInfoAjax(username, password);
});
...
$("#update_profile").click(function() {
  ...
  updateReminderAjax(username, new_password, new_reg_no, new_fname, new_email);
});

// ****************************AJAX****************************
// ajax function to get reminder all data
function getUserInfoAjax(username, password) {
  $.ajax({
    url:
    ...
    error: function() {
      alert(
        "Could not load the data of your Profile from the server please try again"
      );
    }
  });
}
...
function updateReminderAjax(
  ...
  $.ajax({
    url:
      "https://motproject01.pythonanywhere.com/api/update/user/?username=" +
      username +
      "&password=" +
      new_password +
      "&reg_no=" +
    ...
    error: function() {
      alert("Could not update your Profile to the server Please try again");
      // disable the update profile button on start of the page
